...
uint256 hashGenesisBlock( COIN_GENESIS_HASH ); // See config.h
static CBigNum bnProofOfWorkLimit(~uint256(0) >> COIN_DIFFICULTY_SHIFT ); // See config.h
...
int64 CTransaction::nMinTxFee = COIN_MINTXFEE;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = COIN_MINRELAYTXFEE;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy will be halved every 131072nd block until a permanent floor of 27 coins
    // is reached. Ideally, each subsidy level will last for around 40 weeks of mining,
    // and the floor should not be reached sooner than 4.5 years.
    nSubsidy >>= ( nHeight / 131072 );    
    nSubsidy = nSubsidy < 27 ? 27 * COIN : nSubsidy * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = SOLCOIN_OLD_TARGET_TIMESPAN; // For block 0-14999; KGW used for block 15000+
static const int64 nTargetSpacing = COIN_BLOCK_TARGET_TIME;
static const int64 nInterval = COIN_BLOCKS_BETWEEN_RETARGET; // Old diff. retarget
...
// Kimoto Gravity Well, developed for Megacoin by 'Dr. Kimoto Chan' (have a nice day!)
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax)
{
    // current difficulty formula comes from Fedoracoin
    ...
    // debug print
    printf( "Difficulty Retarget - KGW\n" );
...
unsigned int static GetNextWorkRequired( const CBlockIndex* pindexLast, const CBlockHeader *pblock )
{
    ...
    static const int64 BlocksTargetSpacing = COIN_BLOCK_TARGET_TIME;
    uint64 PastBlocksMin                   = COIN_KGW_MIN_PAST_BLOCKS;
    uint64 PastBlocksMax                   = COIN_KGW_MAX_PAST_BLOCKS;

    // Solcoin: Kimoto's Gravity Well applies only to block 15000 and onward
    if( pindexLast->nHeight  < 14999 )
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        /* Reject block.nVersion=1 blocks from block 125k and onwards, even though the Solcoin block chain
           has only one v1 block so far - the genesis block */
        if (nVersion < 2)
        {
            if ( !fTestNet && nHeight >= 125000 )
            {
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = COIN_GENESIS_MSG;
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = COIN_GENESIS_TIME;
        block.nBits    = COIN_GENESIS_BITS;
        block.nNonce   = COIN_GENESIS_NONCE;

        if (fTestNet)
        {
            block.nTime    = 1388122463;
            block.nNonce   = 279743747;
        }
        ...
        assert(block.hashMerkleRoot == uint256( COIN_MERKLE_ROOT ));

        #ifdef MINE_GENESIS_BLOCK
        // Generate a new genesis block if the current one does not match the preset hash.
        // This snippet of code is intentionally left here to help others create their
        // own cryptocurrency; activate via config.h
...
unsigned char pchMessageStart[4] = COIN_PCHMESSAGE_START; // See config.h
